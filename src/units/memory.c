...
MemData mem(unsigned int Address, unsigned int MemRead, unsigned int MemWrite,
    ...
    // Initialize MEM in the heap, only once because I'm unsure if Windows
    // allows 5MB stack allocation. On Linux, where I am creating this, it's
    // configurable.
    if (MEM == NULL) {
...
void write_mem_stats_section(FILE *fstatsptr) {
    ...
    for (int i = FROM_IDX; i < TO_IDX; i += 4) {
        ...
        // Most memory addresses contain zeroes, exclude them to reduce
        // clutter
        if (val != 0) {
